...
use crate::math::{MathNode, NodeKind, Op};
        ...
        let token = match chars.next() {
            ...
            Some(first) => match first {
                ...
                a if a.is_alphabetic() && a.is_lowercase() => {
                    ...
                    ident.push(a);
                    while let Some(a) = chars.next_if(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || *c == '_') {
                        ident.push(a);
                    }
                ...
                a if a.is_alphabetic() && a.is_uppercase() => {
                    ...
                    var.push(a);
                    while let Some(a) = chars.next_if(|c| c.is_ascii_uppercase() || c.is_ascii_digit() || *c == '_') {
                        var.push(a);
                    }
...
pub type Expr = egg::RecExpr<MathNode>;
pub type Pattern = egg::Pattern<MathNode>;
...
pub fn parse_pattern(input: &str) -> Result<Pattern, ParseError> {
    ...
    parse_inner(&mut tokens, &mut pat, 0)?;
    Ok(Pattern::new(pat))
}
...
pub fn parse_expr(input: &str) -> Result<Expr, ParseError> {
    ...
    let mut pat = PatternAst::default();
    parse_inner(&mut tokens, &mut pat, 0)?;
    pat.try_into().map_err(ParseError::VariableInExpr)
}

fn parse_inner<I: Iterator<Item = Token>>(
    tokens: &mut Peekable<I>, 
    pat: &mut PatternAst<MathNode>, 
    min_bp: u8
    ...
    let mut lhs = match tokens.next() {
        Some(T::Num(num)) => {
            let node = MathNode::leaf(NodeKind::Number(num));
            pat.add(ENodeOrVar::ENode(node))
        },
        Some(T::Ident(sym)) => {
            let node = MathNode::leaf(NodeKind::Symbol(sym.into()));
            pat.add(ENodeOrVar::ENode(node))
        },
        Some(T::Var(var)) => {
            pat.add(ENodeOrVar::Var(var))
        }
        Some(T::LParen) => {
            let lhs = parse_inner(tokens, pat, 0)?;
            assert_eq!(tokens.next(), Some(T::RParen));
            lhs
        }
        Some(T::Op(op)) => {
            let r_bp = op.prefix_binding_power()
                .ok_or(ParseError::InvalidPrefixOp(op))?;
            let rhs = parse_inner(tokens, pat, r_bp)?;
            let node = MathNode::new(NodeKind::Operator(op), smallvec![rhs]);
            pat.add(ENodeOrVar::ENode(node))
        }
    ...
    loop {
        ...
        let rhs = parse_inner(tokens, pat, r_bp)?;
        let node = MathNode::new(NodeKind::Operator(op), smallvec![lhs, rhs]);
        lhs = pat.add(ENodeOrVar::ENode(node))
    }
